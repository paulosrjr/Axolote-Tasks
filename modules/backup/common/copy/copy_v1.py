...
class CopyToBackup:
    """
    Execute a copy in remote target
    """

    def __init__(self, **kwargs):
        self._kwargs = kwargs
        self.backup_type = self._kwargs.get('backup_type', 'F')
        self.storage_type = self._kwargs.get('storage_type', 'remote')
    ...
    def _execute_scp_with_pass(self):
        ...
        if self._check_sshpass():
            if self.storage_type == "remote":
                command = "sshpass -p '{}' ssh -p {} -A -o stricthostkeychecking=no -t {}@{} 'scp {} {} {}; if [ \"$?\" == \"0\" ]; then exit 0; else exit 1; fi'".format(
                    self.password,
                    self.port,
                    self.username,
                    self.host,
